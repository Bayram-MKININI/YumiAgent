...
suspend fun <T> FlowCollector<Resource<T>>.handleSessionWithNoFailure(
    ...
    val serviceError = session?.let {
        ...
        ServiceError.ErrNone
    } ?: ServiceError.ErrSystem()
    error?.let { errorDTO ->
        emit(
            Resource.Error(
                serviceError = when (serviceError) {
                    is ServiceError.ErrSystem -> serviceError.copy(errorMessage = errorDTO.errorMessage)
                    else -> serviceError
                },
...
inline fun <reified T : Any, reified K : Any> handlePagingSourceError(
    ...
    try {
        when (ex1) {
            is HttpException -> ServiceError.ErrSystem()
            is IOException -> ServiceError.ErrNetwork
            else -> null
        }?.let { errorType ->
